...
- 検索とフィルタリング
  - キーワード検索：氏名・メールアドレス・部署などを対象とした部分一致検索
  - 権限フィルタ：管理者、マネージャー、一般ユーザーなど権限種別での絞り込み
...
## 4. 画面構成（機能レベル）
- **検索エリア**
  - キーワード検索テキストフィールド：氏名、メールアドレス、部署などを対象に部分一致で検索
  - 権限フィルタドロップダウン：全て／管理者／マネージャー／一般ユーザーから選択
  - 検索条件の変更は即座にサーバーに送信され、結果が一覧に反映される
- **操作エリア**
  - ユーザー追加ボタン、更新ボタン、CSV ダウンロードボタンなどの主要アクションをまとめて配置。
- **ユーザー一覧領域**
...
| 項目名 | フィールドキー | 検索方式 | 備考 |
| --- | --- | --- | --- |
| キーワード | `keyword` | 部分一致 | 氏名、メールアドレス、部署を対象 |
| 権限 | `role` | 完全一致 | `ALL`, `ADMIN`, `MANAGER`, `USER` |
...
3. サーバー側で Prisma を使用してデータベースクエリを実行
   - キーワード検索は `contains` モードで `name`, `email`, `department` に OR 条件を適用
   - 権限フィルタは `role` フィールドに完全一致条件を適用
4. 検索結果をページング情報と共にクライアントに返却
5. フロントエンドで結果を一覧表示

### 9.3 技術的考慮事項

- 検索クエリのパフォーマンス確保のため、`name`, `email` カラムにインデックスを設定
- 大量データ対応として、結果はページング（デフォルト 20 件/ページ）で返却
- 検索条件はクエリパラメータに保持し、ブラウザの戻る/進むに対応

## 10. 今後の拡張（参考）
- ステータスによるフィルタリング機能。
- 高度な検索（作成日範囲、最終ログイン日など）。
- バルク操作（複数ユーザーの一括管理）。
